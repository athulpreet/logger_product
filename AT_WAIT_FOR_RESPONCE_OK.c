...
/**
  ...
  *
  * An additional task, vAtCommandTask, has been added to send "AT"
  * and the receiver task now checks for an "OK" response.
  *
  ******************************************************************************
  */
...
void vAtCommandTask(void *pvParameters); // New task to send AT command
...
void vUart4RxTask(void *pvParameters)
{
    ...
    while(1)
    {
        // Wait indefinitely for a signal from the RxEventCallback
        if (xSemaphoreTake(g_uart4_rx_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            // Use a critical section to safely copy the shared global buffer
            taskENTER_CRITICAL();
            ...
            {
                // Null-terminate the local copy to treat it as a string
                local_buffer[local_bytes_received] = '\0';
                DEBUG_PRINTF("UART4 RX (%u bytes): %s\r\n", local_bytes_received, (char*)local_buffer);

                // *** NEW: Check if the response contains "OK" ***
                if (strstr((char*)local_buffer, "OK") != NULL)
                {
                    DEBUG_PRINTF("SUCCESS: 'OK' response received!\r\n");
                }
...
/**
 * @brief Task to send an "AT" command to UART4 after a delay.
 */
void vAtCommandTask(void *pvParameters)
{
    // Wait for 5 seconds after startup to ensure everything is initialized
    vTaskDelay(pdMS_TO_TICKS(5000));

    const char* at_command = "AT\r\n";
    DEBUG_PRINTF("Sending command >> %s", at_command);

    // Transmit the command over UART4
    HAL_UART_Transmit(&huart4, (uint8_t*)at_command, strlen(at_command), HAL_MAX_DELAY);

    // This task has done its job, so it can delete itself.
    // If you wanted to send commands periodically, you would put this in a loop.
    vTaskDelete(NULL);
}
...
int main(void)
{
  ...
  // Create the three tasks for our application
  xTaskCreate(vDebugLogTask, "DebugLog", 512, NULL, 1, NULL);
  xTaskCreate(vUart4RxTask, "Uart4Rx", 512, NULL, 2, NULL);
  xTaskCreate(vAtCommandTask, "AtCommand", 256, NULL, 2, NULL); // New task added here
